...
export class PlaylistService {
    ...
    async create(playlist: CreatePlaylistDto): Promise<CreatePlaylistResponseDto> {
        ...
        try {
            ...
            const playlistExists: boolean = await this.validateIfPlaylistExistAlready(playlist.title, user.id);
            if (playlistExists) {
                return {
                    newPlaylist: null,
    ...
    async getAllPublic(page: number, limit: number): Promise<PlaylistsResponseDto> {
        const offset = (page - 1) * limit;
        const playlists: Playlists = await this.playlistRepository.getAllPublicPlaylists(offset, limit);
    ...
    async getAllOwn(emailOwner: OwnerDto, page: number, limit: number): Promise<PlaylistsResponseDto> {
        ...
        const offset = (page - 1) * limit;
        const playlists: Playlist[] = await this.playlistRepository.getPlaylistsByUserId(user.id, offset, limit);
    ...
    async getPlaylistByEmail(owner: OwnerDto, playlist: PlaylistDto): Promise<PlaylistResponseDto> {
        const user = await this.userService.findByEmail(owner.email)
        if (!user) return { playlist: null, message: `User does not exist`, error: true }

        const foundPlaylist: Playlist = await this.playlistRepository.getPlaylistByTitle(playlist.title, user.id);
        if (!foundPlaylist) {
            return {
                playlist: null,
    ...
    async update(updatePlaylistDto: UpdatePlaylistDto): Promise<PlaylistResponseDto> {
        const { email, title } = updatePlaylistDto;

        const owner = { email } as OwnerDto;
        const playlist = { title } as PlaylistDto

        const foundPlaylist: PlaylistResponseDto = await this.getPlaylistByEmail(owner, playlist);
        if (!foundPlaylist.playlist) return foundPlaylist;

        const playlistToUpdate: Playlist = this.buildNewPlaylistUpdated(updatePlaylistDto, foundPlaylist.playlist.id);
        await this.playlistRepository.update(playlistToUpdate);

        const updatedPlayList = { title: playlistToUpdate.title } as PlaylistDto
        const playlisUpdated: PlaylistResponseDto = await this.getPlaylistByEmail(owner, updatedPlayList);
    ...
    private async validateIfPlaylistExistAlready(title: string, userId: number) {
        const playListFound: Playlist = await this.playlistRepository.getPlaylistByTitle(title, userId);
    ...
    private buildNewPlaylistUpdated(playlist: UpdatePlaylistDto, id: number): Playlist {
        return {
            id: id,
